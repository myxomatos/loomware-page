...
2. **Jalar.** `git fetch origin && git pull` (la rama personal) y después `git merge origin/main`
   para traer lo que ya entró a producción. Si el merge da conflicto, cárgale
   `mattpocock-skills:resolving-merge-conflicts`, confirma con el usuario lo que no sea obvio y
   termina el merge antes de seguir.
